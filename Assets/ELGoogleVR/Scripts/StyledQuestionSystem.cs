...
public class StyledQuestionSystem : MonoBehaviour
{
    ...
    public QuestionSystemStyle style;
    ...
    public ArabicTextMeshProUGUI startActionText;
	...
	{
        flipper.GetComponent<Image>().sprite = style.background;
        startPanel.GetComponent<Image>().sprite = style.background;
        questionsPanel.GetComponent<Image>().sprite = style.background;
        finishPanel.GetComponent<Image>().sprite = style.background;
        rewardPanel.GetComponent<Image>().sprite = style.background;
        rewardImage.sprite = style.rewardBackground;
        ...
        ResetAlignment(style.layoutDirection);
    }

	private void ResetAlignment(QuestionSystemStyle.LayoutDirection layoutDirection)
	{
		Vector3 alignmentScale = layoutDirection == QuestionSystemStyle.LayoutDirection.LTR ? ltrScale : rtlScale;
		bool fix = layoutDirection == QuestionSystemStyle.LayoutDirection.LTR ? false : true;
        ...
        startMessageText.TextMeshProUGUI.font = style.font;
        startMessageText.TextMeshProUGUI.color = style.colorStartMessage;
        ...
        if (startActionText.TextMeshProUGUI == null)
        {
            startActionText.TextMeshProUGUI = startActionText.GetComponent<TextMeshProUGUI>();
        }
        startActionText.TextMeshProUGUI.font = style.font;
        startActionText.TextMeshProUGUI.color = style.colorStartAction;
        startActionText.fix = fix;
        startActionText.TextMeshProUGUI.isRightToLeftText = fix;
        ...
        finishMessageText.TextMeshProUGUI.font = style.font;
        finishMessageText.TextMeshProUGUI.color = style.colorFinish;
        ...
        pointsText.TextMeshProUGUI.font = style.font;
        pointsText.TextMeshProUGUI.color = style.colorReward;
        ...
        questionText.TextMeshProUGUI.font = style.font;
        questionText.TextMeshProUGUI.color = style.colorQuestion;
		...
		for (int i = 0; i < choiceText.Length; i++)
		{
            ...
            choiceText[i].TextMeshProUGUI.font = style.font;
            choiceText[i].TextMeshProUGUI.color = style.colorChoice;
	...
	private float PlayFeedback(bool isCorrect)
	{
		...
		if (isCorrect && style.correctAnswerFeedbacks != null && style.correctAnswerFeedbacks.Length > 0)
		{
			return PlayAudioClip(style.correctAnswerFeedbacks[Random.Range(0, style.correctAnswerFeedbacks.Length)]);
		}
		else if (!isCorrect && style.wrongAnswerFeedbacks != null && style.wrongAnswerFeedbacks.Length > 0)
		{
			return PlayAudioClip(style.wrongAnswerFeedbacks[Random.Range(0, style.wrongAnswerFeedbacks.Length)]);
		}
	...
	IEnumerator ShowingStart()
	{
        ...
        startMessageText.Text = style.startMessageTAblock.text;
        startMessageText.gameObject.SetActive(true);

        yield return new WaitForSeconds(PlayAudioClip(style.startMessageTAblock.audio));

        startActionImage.sprite = style.startActionSprite;
        startActionText.Text = style.startActionTAblock.text;
	...
	IEnumerator ShowingFinish(bool didGreat)
	{
        ...
        if (didGreat)
        {
            finishMessageText.Text = style.finishMessageGoodTAblock.text;
            finishMessageText.gameObject.SetActive(true);
            yield return new WaitForSeconds(PlayAudioClip(style.finishMessageGoodTAblock.audio));
        }
        else
        {
            finishMessageText.Text = style.finishMessageBadTAblock.text;
            finishMessageText.gameObject.SetActive(true);
            yield return new WaitForSeconds(PlayAudioClip(style.finishMessageBadTAblock.audio));
        }
	...
	IEnumerator ShowingQuestion()
	{
		...
		if (currentQuestion.type == TextQuestion.Type.MultipleChoice)
		{
			...
			for (int i = 0; i < currentQuestion.choices.Count; i++)
			{
				...
				choiceButtons[i].image.sprite = style.choiceBackgroundSprites[normal];
		...
		{
			trueButton.image.sprite = style.trueSprites[normal];
			trueButton.interactable = trueButton.image.raycastTarget = true;
			trueButton.gameObject.SetActive(true);
			yield return new WaitForSeconds(PlayAudioClip(style.choiceTrue));

			falseButton.image.sprite = style.falseSprites[normal];
			falseButton.interactable = falseButton.image.raycastTarget = true;
			falseButton.gameObject.SetActive(true);
            yield return new WaitForSeconds(PlayAudioClip(style.choiceFalse));
        }
		eventSystem.SetActive(true);
	}
	...
	IEnumerator Correctifying(bool isTrue)
	{
		...
		if (isTrue)
		{
			...
			{
				trueButton.image.sprite = style.trueSprites[correct];
			...
			{
				trueButton.image.sprite = style.trueSprites[wrong];
				falseButton.image.sprite = style.falseSprites[correct];
		...
		else
		{
			...
			{
				falseButton.image.sprite = style.falseSprites[wrong];
				trueButton.image.sprite = style.trueSprites[correct];
	...
	IEnumerator ValidatingChoice(int choiceIndex)
	{
		...
		{
			choiceButtons[choiceIndex - 1].image.sprite = style.choiceBackgroundSprites[correct];
		...
		else
		{
			...
			{
				choiceButtons[choiceIndex - 1].image.sprite = style.choiceBackgroundSprites[wrong];
				choiceButtons[choiceIndex - 1].interactable = choiceButtons[choiceIndex - 1].image.raycastTarget= false;
				...
				for (int i = 0; i < choiceButtons.Length; i++)
				{
					...
					{
						choiceButtons[i].image.sprite = style.choiceBackgroundSprites[correct];
						choiceButtons[i].interactable = choiceButtons[i].image.raycastTarget = false;
						break;
					}
				}
